/**
 * ARCHIVO: auth.service.ts - Servicio de autenticación JWT
 *
...
// Es como un tubo por donde fluyen datos: puede emitir varios valores a lo largo del tiempo.
...
export class AuthService {
   ...
   *
   * Observable<AuthResponse>: Indica que esta función devuelve un Observable
   * que emitirá la respuesta del servidor cuando se suscriba.
   */
  login(data: LoginRequest): Observable<AuthResponse> {
    // .pipe(tap(...)): "tubería" que ejecuta código después de recibir la respuesta
    // sin modificar la respuesta en sí.
    return this.http.post<AuthResponse>(`${this.api}/login`, data).pipe(
      tap((res) => {
        // Guardamos el token JWT en el almacenamiento local del navegador.
        localStorage.setItem('token', res.token);
        // Si el servidor envía un refreshToken, también lo guardamos.
        if (res.refreshToken) localStorage.setItem('refreshToken', res.refreshToken);
        // Actualizamos los signals para que Angular refleje el nuevo estado.
   ...
   *
   * Elimina los tokens del localStorage y resetea los signals a su estado inicial.
   */
  logout(): void {
